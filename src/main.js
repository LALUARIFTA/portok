  ...
  try {
    const doTrack = async () => {
      try {
        await supabase
          .from("analytics")
          .insert([{ event_name: eventName, event_data: eventData }]);
      } catch (err) {
        /* silent fail */
      }
    };
...
function updateArticleSEO(article) {
  ...
  const schemaTag = document.getElementById("articleSchema");
  if (schemaTag) {
...
function buildWhatsAppLink(
  phone,
  message = "Halo Ayek, saya tertarik untuk kolaborasi.",
) {
  if (!phone) return "";
...
function applyLeadContactLinks(whatsapp) {
  const waUrl = buildWhatsAppLink(whatsapp);
  if (!waUrl) return;
  leadContact.whatsappUrl = waUrl;
  ...
  const mobileWaCta = document.getElementById("mobileWaCta");
  if (mobileWaCta) mobileWaCta.href = waUrl;

  const successWa = document.getElementById("contactSuccessWaLink");
  ...
  const commands = {
    ...
    "ls projects": () =>
      allProjects.length > 0
        ...
        : "Belum ada proyek yang dimuat.",
    socials: () =>
      'LinkedIn: <a href="https://linkedin.com/in/lalu-arif" target="_blank" style="color:var(--accent);">lalu-arif</a>\nGitHub: <a href="#" target="_blank" style="color:var(--accent);">ayek-dev</a>',
    contact: () =>
      'Email: <span style="color:#eab308">gawah@example.com</span> (atau gunakan form di bawah).',
    sudo: () =>
      "nice try, but you are not in the sudoers file. This incident will be reported. 🚨",
    "sudo rm -rf /": () =>
      '<span style="color: red;">[ERROR] Permission denied. Don\'t try to destroy my portfolio!</span>',
    astrid: () =>
      '<span style="color: #ec4899;">💖 Astrid terdeteksi! Pasangan dan istri tercinta Ayek. Have a great day!</span>',
    play: () =>
      'Loading mini-game... <br>[█░░░░░░░░░] 10%<br>[████░░░░░░] 40%<br>[█████████░] 90%<br><span style="color: red;">[FATAL ERROR] Not enough RAM to run Doom.</span>',
    matrix: () =>
      '<span style="color: #22c55e;">Wake up, Neo...<br>The Matrix has you...<br>Follow the white rabbit. 🐇</span>',
    secret: () =>
      "🎉 <b>Easter Egg Ditemukan!</b> Anda memang teliti. Sebagai hadiah, ini sebuah pantun untuk Anda:<br><i>Jalan-jalan ke kota tua,<br>Jangan lupa membeli kaca.<br>Website ini memang biasa,<br>Tapi dibuat sepenuh jiwa.</i>",
...
window.openProjectDetail = async (id) => {
  ...
  document.getElementById("pmCategory").textContent =
    p.category === "certificate" ? "Sertifikat" : p.category;
  document.getElementById("pmYear").textContent = p.year || "-";
  document.getElementById("pmStack").textContent = p.tech_stack || "-";
  document.getElementById("pmDescription").textContent =
    p.case_study || p.description || "No detailed case study yet.";
  ...
  if (p.gallery && p.gallery.length > 0) {
    gallery.innerHTML = p.gallery
      .map(
        (img) =>
          `<img src="${img}" alt="Gallery image for ${p.title}" loading="lazy">`,
      )
      .join("");
  } else {
    gallery.innerHTML = "";
...
async function loadDynamicContent() {
  try {
    ...
    if (p) {
      ...
      if (elTermWhoami)
        elTermWhoami.textContent = `${p.name.toLowerCase()} — IT Professional`;
      ...
      if (p.whatsapp) applyLeadContactLinks(p.whatsapp);
      ...
      if (socials) {
        const linkStyle =
          "display:inline-flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:8px;border:1px solid var(--border);color:var(--text-muted);text-decoration:none;transition:all 0.2s;";
        const hoverIn =
          "this.style.borderColor='var(--accent)';this.style.color='var(--accent)';this.style.transform='translateY(-2px)'";
        const hoverOut =
          "this.style.borderColor='var(--border)';this.style.color='var(--text-muted)';this.style.transform='translateY(0)'";
        let html = "";
        if (p.email)
          html += `<a href="mailto:${p.email}" style="${linkStyle}" onmouseover="${hoverIn}" onmouseout="${hoverOut}" aria-label="Email"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg></a>`;
        if (p.github)
          html += `<a href="${p.github}" target="_blank" style="${linkStyle}" onmouseover="${hoverIn}" onmouseout="${hoverOut}" aria-label="GitHub"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.4 5.4 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65S8.93 17.38 9 18v4"/><path d="M9 18c-4.51 2-5-2-7-2"/></svg></a>`;
        if (p.linkedin)
          html += `<a href="${p.linkedin}" target="_blank" style="${linkStyle}" onmouseover="${hoverIn}" onmouseout="${hoverOut}" aria-label="LinkedIn"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect width="4" height="12" x="2" y="9"/><circle cx="4" cy="4" r="2"/></svg></a>`;
        if (p.instagram)
          html += `<a href="${p.instagram}" target="_blank" style="${linkStyle}" onmouseover="${hoverIn}" onmouseout="${hoverOut}" aria-label="Instagram"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect width="20" height="20" x="2" y="2" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" x2="17.51" y1="6.5" y2="6.5"/></svg></a>`;
        if (p.whatsapp) {
    ...
    // Experience/Education/Testimonial loaded in parallel
    const [{ data: exp }, { data: edu }, { data: testi }] = await Promise.all([
      supabase
        .from("experience")
        .select("*")
    ...
    if (testiSection && testi && testi.length > 0) {
      ...
      if (track) {
        ...
        while (baseSet.length < 4) {
          // Minimum items to fill a screen width
          baseSet = [...baseSet, ...testi];
        }
        const displayData = [...baseSet, ...baseSet];

        track.innerHTML = displayData
          .map(
            (t) => `
...
function updateStats() {
  ...
  if (sProj)
    sProj.textContent = allProjects.filter((p) => p.category === "web").length;
  if (sDes)
    sDes.textContent = allProjects.filter(
      (p) => p.category === "design",
    ).length;
  if (sNet)
    sNet.textContent = allProjects.filter(
...
async function loadSingleArticle() {
  ...
  try {
    ...
    document.getElementById("articleDate").textContent = new Date(
      article.created_at,
    ...
    if (typeof marked !== "undefined") {
      ...
      bodyEl.innerHTML = sanitizeHTML(parsed);
    } else {
...
async function initChatbot() {
  ...
  async function handleChat(text) {
    ...
    try {
      ...
      5. Jika ditanya kontak, arahkan pengguna ke section Contact pada website.
      ...
      botMsgEl.textContent =
        "Maaf, sepertinya saya sedang offline. Silakan coba lagi nanti atau hubungi Ayek langsung melalui email.";
    }
  }
  ...
  function detectCTA(userText, botReply) {
    ...
    if (
      ...
      combined.includes("riwayat hidup")
    ) {
    ...
    // 2. Contact/Hire
    if (
      combined.includes("kontak") ||
      combined.includes("contact") ||
      combined.includes("hubung") ||
...
function initContact() {
  ...
  const charCountEl = document.getElementById("contactCharCount");
  ...
  if (messageInput) {
    ...
    const updateCounter = () => {
      ...
      charCountEl.textContent = `${messageInput.value.length}/${maxMessageLength}`;
    };
    updateCounter();
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!turnstileResponse) {
      showCustomAlert(
        "warning",
        "Harap selesaikan verifikasi keamanan (CAPTCHA).",
      );
      return;
    }
    ...
    try {
      ...
      if (leadContact.whatsappUrl) {
        trackEvent("lead_message_sent", { source: "contact_form" });
      }
      ...
      form.reset();
      if (charCountEl) charCountEl.textContent = `0/${maxMessageLength}`;
    } catch (err) {
      console.error(err);
      showCustomAlert(
        "error",
...
function initLeadConversion() {
  const waButtons = [
    ...
    document.getElementById("mobileWaCta"),
    document.getElementById("contactSuccessWaLink"),
  ].filter(Boolean);

  waButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      trackEvent("lead_whatsapp_click", { source: btn.id || "wa_cta" });
    });
  });
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  runWhenVisible(
    ...
    () => {
      import("./three-bg.js")
        .then(({ initThreeJSBackground }) => {
          // Keep effect focused on hero to reduce GPU cost.
          initThreeJSBackground("#hero");
        })
      ...
      import("./pixel-art.js")
        .then(({ PixelatedCanvas }) => {
          new PixelatedCanvas({
            ...
            height: isMobile ? 1000 : 1500,
            cellSize: isMobile ? 7 : 6,
            dotScale: 0.9,
            shape: "square",
            ...
            distortionRadius: isMobile ? 90 : 120,
            distortionMode: "swirl",
            followSpeed: 0.15,
            jitterStrength: isMobile ? 1.4 : 2,
            jitterSpeed: 2,
            sampleAverage: true,
            tintStrength: 0,
          });
...
class TextScramble {
  ...
  async scramble() {
    ...
    const originalText =
      this.el.getAttribute("data-scramble-text") || this.el.innerText;
        ...
        if (progress * originalText.length > i) {
          ...
          scrambled +=
            this.options.characterSet[
              Math.floor(Math.random() * this.options.characterSet.length)
            ];
        }
      }
...
function initLightbox() {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (
      ...
      target.parentElement.id === "pmGallery" &&
      target.tagName === "IMG"
    ) {
      const allImgs = Array.from(
        document.querySelectorAll("#pmGallery img"),
      ).map((img) => img.src);
    ...
    if (
      target.parentElement &&
      target.parentElement.id === "pmHero" &&
      target.tagName === "IMG"
    ) {
...
function renderPagination(totalItems) {
  ...
  const showLeftEllipsis =
    currentProjectPage - 1 > paginationItemsToDisplay / 2;
  const showRightEllipsis =
    totalPages - currentProjectPage + 1 > paginationItemsToDisplay / 2;
